...
public class LoginController extends BaseController {
	...
	public void index() {
		createToken();
		setAttr("returnUrl",encodeReturnUrl(getPara("returnUrl","")));		//没事了//修改，原来是没有
		render("login.html");
	}
	...
	public void submit() {
		...
		String returnUrl=encodeReturnUrl(getPara("returnUrl","/pub/backstage"));
	
		try {
			password = RSAKit.decryptionToString(password);
			loginService.aopLogin(userCode, password, getRequest());
			redirect(!returnUrl.equals("")?returnUrl:"/pub/backstage");  //修改 原来是根目录
		} catch (Exception e) {
			handerException(e);
